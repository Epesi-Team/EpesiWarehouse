...
//              $item = Utils_RecordBrowserCommon::get_record('premium_warehouse_items', $row['item_id']);
    ...
    public function attachment_addon($arg){
        $a = $this->init_module('Utils/Attachment',array('premium_warehouse_distributor/'.$arg['id']));
...
//              $item = Utils_RecordBrowserCommon::get_record('premium_warehouse_items', $row['item_id']);
                ...
                $form = $this->init_module('Libs/QuickForm');
                $field = 'link_it_'.$row['id'];
                $form->addElement('autocomplete', $field, '', array($this->get_type().'Common', 'item_match_autocomplete'), array($row['distributor_id']),array('id'=>'item_id_autocomplete_'.$row['id']));
                $form->addElement('submit', 'submit', '');
                $theme = $this->init_module('Base/Theme');
                $form->assign_theme('form', $theme);
                $theme->assign('field_name', $field);
                ...
                if ($form->validate()) {
                    ...
                    $sku =  '<span id="link_it_'.$row['id'].'_form" style="display:none;">'.
                            ...
                            '<span id="link_it_'.$row['id'].'_choice">'.
                                ...
                                '<a '.$lp->get_href(array($row['id'])).' onMouseUp="$(\'add_item_name\').value=\''.htmlspecialchars(escapeJS($item_name,false)).'\';var rd=$(\'3rdp_info_frame\');if(rd)rd.innerHTML=\''.htmlspecialchars(escapeJS($row['3rdp']?$row['3rdp']:'queued to fetch',false)).'\';$(\'add_item_man\').innerHTML=\''.htmlspecialchars(escapeJS($row['manufacturer_name'],false)).'\';$(\'add_item_mpn\').value=\''.htmlspecialchars(escapeJS($row['manufacturer_part_number'],false)).'\';$(\'add_item_upc\').value=\''.htmlspecialchars(escapeJS($row['upc'],false)).'\';focus_by_id(\'add_item_name\');"><img src="'.Base_ThemeCommon::get_template_file('Premium/Warehouse/Wholesale','add_item.png').'" border="0" /></a>'.
                            '</span>';
                }
            }
            $gb->add_row(

...
class Premium_Warehouse_Wholesale__Plugin_techdata implements Premium_Warehouse_Wholesale__Plugin {
	...
	public function download_file($parameters, $distributor) {
		...
		if (!isset($match[1])) {
			Premium_Warehouse_WholesaleCommon::file_download_message(__('Unable to locate file to download.'), 2, true);
			return false;
		}
			...
			if ($row_parts['W_MAG']!='nie_ma' && $row_parts['NAZWA']) {
				...
				if($row_parts['GRUPA']) {
					if(!isset($categories[$row_parts['GRUPA']])) {
						$categories[$row_parts['GRUPA']] = Utils_RecordBrowserCommon::new_record('premium_warehouse_distr_categories',array('foreign_category_name'=>$row_parts['GRUPA'],'distributor'=>$distributor['id']));
						$new_categories++;
				...
				if (($internal_key===false || $internal_key===null)) {
					if ($w_item!==null) {
						...
						DB::Execute('INSERT INTO premium_warehouse_wholesale_items (internal_key, distributor_item_name, distributor_id, quantity, quantity_info, price, price_currency, distributor_category,manufacturer,manufacturer_part_number) VALUES (%s, %s, %d, %d, %s, %f, %d, %d, %d, %s)', array($row_parts['KOD_TD'], $row_parts['NAZWA'], $distributor['id'], $quantity, $quantity_info, $row_parts['CENA_C'], $pln_id,$category,$manufacturer, substr($row_parts['SYMBOLPROD'],0,32)));
					}
				} else {

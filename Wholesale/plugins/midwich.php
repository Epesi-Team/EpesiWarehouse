...
class Premium_Warehouse_Wholesale__Plugin_midwich implements Premium_Warehouse_Wholesale__Plugin {
	...
	public function download_file($parameters, $distributor) {
		...
		if(!file_exists($dir.'/midwich/Midwich.csv')) {
			Premium_Warehouse_WholesaleCommon::file_download_message(__('No CSV file in ZIP.'), 2, true);
	...
	public function update_from_file($filename, $distributor,$params) {
		...
		if ($pln_id===false || $pln_id===null) {
			Premium_Warehouse_WholesaleCommon::file_scan_message(__('Unable to find required currency (%s), aborting.', array('GBP')), 2, true);
			return false;
		}
		...
		while (!feof($f)) {
			...
			if (($internal_key===false || $internal_key===null)) {
				...
				if ($w_item!=null)
					DB::Execute('UPDATE premium_warehouse_wholesale_items SET item_id=%d WHERE internal_key=%s AND distributor_id=%d', array($w_item, $row['midw_part_no'], $distributor['id']));
			}
		} 

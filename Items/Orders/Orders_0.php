...
//	    	if (!isset($disabled['disable_rental'])) $defaults[__('Rental')] = array('icon'=>Base_ThemeCommon::get_template_file($this->get_type(),'rental.png'), 'defaults'=>array_merge($defaults,array('transaction_type'=>3,'transaction_date'=>date('Y-m-d'))));
    ...
    public function browse_mode_details(& $form, & $external_filters, & $vals, & $crits, $removed_param, $rb_obj = null) {
		if (!isset($rb_obj)) $rb_obj = $removed_param; // TODO: remove this fallback in the future. 2015-09-03
...
//							array('field'=>'quantity', 'width'=>10)
	...
	public function transaction_history_addon($arg){
		...
		$order = array(array('item_name'=>$arg['id']), array('quantity_on_hand'=>false)+('item_name'=='item_name'?array('item_name'=>false):array()), array('transaction_id'=>'DESC'));
		$rb->set_button(false);

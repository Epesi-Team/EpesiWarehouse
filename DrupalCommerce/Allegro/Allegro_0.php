...
//		$ret = DB::Execute($query);
		...
		while($row=$ret->FetchRow()) {
			...
			$gb_row->add_action('onClick="Premium_Warehouse_DrupalCommerce_Allegro_wystaw(this.parentNode,'.$row['id'].')" href="javascript:void(0);"','Edit','Wystaw');
			$gb_row->add_action('onClick="Premium_Warehouse_DrupalCommerce_Allegro_usun(this.parentNode,'.$row['id'].')" href="javascript:void(0);"','Delete','Usuń z listy');
		}
		$this->display_module($gb);
		load_js($this->get_module_dir().'utils.js');
		eval_js('jQuery(".allegro_konkurencja").each(function(key,el) { Premium_Warehouse_DrupalCommerce_Allegro_konkurencja(jQuery(el));});');
		eval_js('jQuery(".allegro_konkurencja_field").change(function() { Premium_Warehouse_DrupalCommerce_Allegro_konkurencja_field(jQuery(this));});');
		eval_js('jQuery(".allegro_koszt").each(function(key,el) { Premium_Warehouse_DrupalCommerce_Allegro_koszt(jQuery(el));});');
	}
...
//		if(empty($cats))
//			$cats = Premium_Warehouse_DrupalCommerce_AllegroCommon::update_cats();
...
//		$qf->addElement('select','category',$this->t('Kategoria'),$cats);
		...
		} elseif($price = DB::GetOne('SELECT f_gross_price FROM premium_ecommerce_prices_data_1 WHERE f_product=%d AND f_currency=%d',array($pid,Utils_CurrencyFieldCommon::get_id_by_code('PLN')))) {
		...
		$qf->addElement('select','template',$this->t('Szablon'),Premium_Warehouse_DrupalCommerce_AllegroCommon::get_templates());
		...
		if($qf->validate()) {
			...
			$fields = Premium_Warehouse_DrupalCommerce_AllegroCommon::get_publish_array($r,$vals,$auction_cost);
			if(!$fields) return;
...
//				error_log(print_r($ret,true),3,'/tmp/dupa124');
			...
			Premium_Warehouse_DrupalCommerce_AllegroCommon::delete_photos();
		}
...
//		$query = 'SELECT a.item_id,a.ended_on FROM premium_ecommerce_allegro_auctions a WHERE a.active=0 AND ended_on is not null ORDER BY a.ended_on DESC LIMIT 10';
	...
	public function admin() {
		...
		$settings[] = array('name'=>'login','label'=>'Login','type'=>'text','default'=>'','rule'=>$rule);
   		...
   		$settings[] = array('name'=>'template','label'=>'Szablon','type'=>'select','values'=>Premium_Warehouse_DrupalCommerce_AllegroCommon::get_templates(),'default'=>'');

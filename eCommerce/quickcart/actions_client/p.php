...
if( isset( $iContent ) && is_numeric( $iContent ) ){
  ...
  if( isset( $aData ) ){
    ...
    if( !empty( $aData['sTemplate'] ) )
      ...
      if( $config['inherit_themes'] === true && !empty( $aData['iPageParent'] ) ){
    ...
    $sProductsList = ( $aData['iProducts'] == 1 || ( isset( $sPhrase ) && $config['page_search'] == $iContent ) ) ? $oProduct->listProducts( $aData['sTemplate'], $iContent ) : null;

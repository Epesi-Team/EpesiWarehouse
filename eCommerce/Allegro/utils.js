function Premium_Warehouse_eCommerce_Allegro_konkurencja(el) {
    jQuery.get('modules/Premium/Warehouse/eCommerce/Allegro/konkurencja.php',{name:el.attr('name'),cat:el.attr('cat'),id:el.attr('auction_id')},function(data){
	el.html(data);
    });
}
function Premium_Warehouse_eCommerce_Allegro_konkurencja_field(el2) {
    var n = el2.val();
    var k = el2.parent().find('.allegro_konkurencja');
    if(!n) return k.html();
    k.html('...');
    jQuery.get('modules/Premium/Warehouse/eCommerce/Allegro/konkurencja.php',{name:n,cat:k.attr('cat'),id:k.attr('auction_id')},function(data){
	k.html(data);
    });
}
function Premium_Warehouse_eCommerce_Allegro_koszt(el) {
    var auction = el.attr('auction_id');
    jQuery.get('modules/Premium/Warehouse/eCommerce/Allegro/koszt.php',{
	    id:auction,
	    ...
	    el.html(data);
	    Premium_Warehouse_eCommerce_Allegro_profit(auction);
	});
...
function Premium_Warehouse_eCommerce_Allegro_profit(auction) {
...
function Premium_Warehouse_eCommerce_Allegro_wystaw(status,auction) {
    status.innerHTML='...';
    jQuery.get('modules/Premium/Warehouse/eCommerce/Allegro/wystaw.php',{
	    id:auction,
	    buy_price:jQuery('#allegro_buy_price_'+auction).val(),
	    minimal_price:jQuery('#allegro_minimal_price_'+auction).val(),
	    initial_price:jQuery('#allegro_initial_price_'+auction).val(),
...
function Premium_Warehouse_eCommerce_Allegro_usun(status,auction) {
    if(confirm("Usunąć auckję listy do ponownego wystawienia?")) {
      status.innerHTML='...';
      jQuery.get('modules/Premium/Warehouse/eCommerce/Allegro/usun.php',{
	    id:auction,
	},function(data){
	    if(typeof status != "undefined") {
		if(data.indexOf("OK")<0) {

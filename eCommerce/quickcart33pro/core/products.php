  ...
  */
  function getProducts($where = '',$limit=null,$offset=null, $navigation = false){
	...
	$ret = DB::GetAll('SELECT 	it.id as iProduct, 
                    ...
                    GREATEST(IF(SUM(loc.f_quantity)>0,1,0),0,(SELECT 1 FROM premium_warehouse_wholesale_items dist_item
					WHERE dist_item.item_id=it.id AND dist_item.quantity>0 AND dist_item.price_currency='.$currency.' ORDER BY dist_item.price LIMIT 1)) DESC, pr.f_position'.($limit!==null?' LIMIT '.(int)$limit.($offset!==null?' OFFSET '.(int)$offset:''):''));
	$pids = array();
	foreach($ret as $aExp)
		$pids[] = $aExp['iProduct'];
	if($pids)
...
/*	$ret_features = DB::GetCol('SELECT pp.f_item_name
						...
						LEFT JOIN premium_ecommerce_param_group_labels_data_1 gl ON (gl.f_group=g.id AND gl.f_language="'.LANGUAGE.'" AND gl.active=1)

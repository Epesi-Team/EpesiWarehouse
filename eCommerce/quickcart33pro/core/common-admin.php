...
function throwSubpagesShowSelect( $iShow = null ){
  ...
  $aSubpages[4] = $GLOBALS['lang']['Subpage_show_4'];
...
* @param string   $sFile
*/
function listBackupFiles( $sFile ){
  $oDir = dir( DIR_BACKUP );
  while( false !== ( $sFileName = $oDir->read( ) ) ) {
    if( is_file( DIR_BACKUP.$sFileName ) && ereg( '.zip', $sFileName ) ){
      $aFiles[] = $sFileName;
    }
  } // end while
  $oDir->close( );
  ...
  if( isset( $aFiles ) ){
    ...
    $iCount = count( $aFiles );
    for( $i = 0; $i < $iCount; $i++ ){
      $aData['sFile'] = $aFiles[$i];
      $aData['sDate'] = substr( $aFiles[$i], 7, 10 ).' '.ereg_replace( '-', ':', substr( $aFiles[$i], 18, 5 ) );
...
function deleteDbFiles( ){
  $oDir = dir( DIR_DB );
  while( false !== ( $sFileName = $oDir->read( ) ) ) {
    if( is_file( DIR_DB.$sFileName ) ){
      unlink( DIR_DB.$sFileName );
    }
  } // end while
  $oDir->close( );  
} // end function deleteDbFiles

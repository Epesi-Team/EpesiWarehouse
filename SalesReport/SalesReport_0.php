...
class Premium_Warehouse_SalesReport extends Module {
	...
	public function currency_exchange_editor() {
		...
		$query = 'SELECT re.exchange_rate, o.f_transaction_date, od.f_net_price, o.id AS order_id FROM (premium_warehouse_items_orders_details_data_1 AS od LEFT JOIN premium_warehouse_items_orders_data_1 AS o ON o.id=od.f_transaction_id) LEFT JOIN premium_warehouse_sales_report_exchange AS re ON re.order_id=o.id AND re.currency=%d WHERE od.active=1 '.$show_missing.'AND (o.f_transaction_type=0 OR o.f_transaction_type=1) AND f_net_price!=\'\' AND f_net_price '.DB::like().' '.DB::Concat(DB::qstr('%'), DB::qstr('__'.$sel_curr)).' GROUP BY od.f_transaction_id';
...
//							if (!isset($purchase_amount[$currency])) break;
	...
	public function sales_by_item() {
		...
		if ($this->range_type['other']['item_name']) $item_filter = 'AND i.f_item_name '.DB::like().' '.DB::Concat(DB::qstr('%'), DB::qstr($this->range_type['other']['item_name']), DB::qstr('%')).' ';
		else $item_filter = '';
...
/************************************************************************************/
	public function display_sales($warehouse_id, $start, $end) {
		...
		$rb->set_header_properties(array('status'=>array('wrapmode'=>'nowrap')));
